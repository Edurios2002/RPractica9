...
//-------declaraciones de las animaciones.
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (Animacion_Cartel > 2)
		{
			if (!Pausar)
			{
				if (Bajando)
				{
					if (Cartel_Fijo < 3.5f)
					{
						Cartel_Fijo += Cartel_Rotacion * deltaTime;
					}
					else
					{
						Pausar = true;
						Tiempo_Pausa = Animacion_Cartel;
						Esperar_Arriba = true;
						Angulo_Rotacion = 0.0f;
					}
				}
			...
			{
				float tiempoTranscurrido = Animacion_Cartel - Tiempo_Pausa;

				if (tiempoTranscurrido < 2.0f)
				{
					Angulo_Rotacion = 180.0f * (tiempoTranscurrido / 2.0f);
				}
				else
				{
					Angulo_Rotacion = 360.0f;
				}
		...
		/*
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0, Cartel_Fijo, 0.0f));
		model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::translate(model, glm::vec3(0.0f, Cartel_Fijo, 0.0f));  // Mover el letrero a su posición
		model = glm::rotate(model, glm::radians(Angulo_Rotacion), glm::vec3(1.0f, 0.0f, 0.0f));  // Rotar sobre el eje X
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		arco.RenderModel();
		*/
